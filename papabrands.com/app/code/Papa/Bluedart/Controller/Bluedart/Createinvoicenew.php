 ...
 * @author mannu
 */
class Createinvoicenew extends \Magento\Backend\App\Action {
    ...
    public function execute() {
		...
		$_url_details='https://api.bluedart.com/servlet/RoutingServlet?handler=tnt&action=custawbquery&loginid=DDP81791&awb=awb&numbers=78500019824&format=xml&lickey=%20kjkirtetmlnfseekprgg4qjplxluqlle&verno=1.3&scan=1';
		...
		try {
			...
			if ($payment_method_code == 'cashondelivery') {
				$amount_due .='<tr style="display: flex; margin-bottom: 10px;">
					<td></td>
					<td colspan="2" style="font-size: 1.2rem; font-weight: 600;  color: #000; text-transform: uppercase;"><b>AMOUNT DUE:</b></td>
					<td style="font-size: 1.1rem; font-weight: 600; text-align: left; width: 20%;">'.$order->getOrderCurrencyCode().' '.($order->getGrandTotal()*1).'</td>
				</tr>';
			}
...
";
		...
		//die;	
			...
			echo $file_name = 'order_invoice_' .$param['order_id'].'.pdf';
